...
func OverrideCredentials(srcDir string) error {
	...
	srcDestPairs := [3]SourceDestinationPair{
		SourceDestinationPair{filepath.Join(dir, filepath.Base(keyPath)), keyPath},
		SourceDestinationPair{filepath.Join(dir, filepath.Base(certPath)), certPath},
		SourceDestinationPair{filepath.Join(dir, filepath.Base(confPath)), confPath},
	}
	...
	if err != nil {
		return fmt.Errorf("Couldn't load certificate: %w", err)
	}
